...
panel with `tool_choice: "required"` so it always deliberates. This plays to
Fusion's strength — multi-model deliberation on judgment calls — rather than
line-by-line code linting.

## Presets (`--preset` or `FUSION_ADVISOR_PRESET`)
- `general-high` (default) — strongest all-round panel.
- `general-budget` — cheaper panel with a frontier judge; good for routine consults.
...
replace the preset — e.g. non-Anthropic models, so it isn't "Claude advising Claude":
```
...
- **HTTP 400 mentioning `tool_choice`** → remove the `"tool_choice": "required"`
  line in `advise.py`; an advisory prompt still triggers the panel almost always.
